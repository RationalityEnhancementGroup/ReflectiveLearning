/**
 * jspsych-survey-text-validate-numbers
 * a jspsych plugin for free response survey questions
 * with number only input and response validation
 *
 * Valkyrie Felso
 *
...
jsPsych.plugins['survey-text-validate-numbers'] = (function() {
    ...
    $("#jspsych-survey-text-next").click(function() {
      finished = false
        ...
        $("div.jspsych-survey-text-question").each(function(index) {
          valid_fields.push($(this).children('textarea').val().match(/^[0-9]+$/) != null);
        });

        if (valid_fields.indexOf(false) >= 0){
            alert("Please answer each question, using only numbers as input.");}
        else {
          finished=true
        }
