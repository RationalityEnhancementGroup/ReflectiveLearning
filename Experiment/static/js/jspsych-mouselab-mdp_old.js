...
// coffeelint: disable=max_line_<length
mdp = void 0;
...
jsPsych.plugins['mouselab-mdp'] = (function() {
  var Arrow, Edge, KEYS, LOG_DEBUG, LOG_INFO, MouselabMDP, NULL, PRINT, RIGHT_MESSAGE, SIZE, State, Note, TOP_ADJUST, LEFT_ADJUST, TRIAL_INDEX, Text, angle, checkObj, dist, plugin, polarMove, redGreen, removePrivate, round;
  PRINT = function(...args) {
  ...
  MouselabMDP = class MouselabMDP {
    constructor(config) {
      ...
      if (this.energyLimit) {
        ...
        if (this.denom && !(this.start_trial === null)) {
          leftMessage = "Round " + (this._block.trialCount + 1 + this.start_trial) + "/" + this.denom;
        } else {
      ...
      if(this.show_interactive_flowchart){
        ...
        $('<div>', {
          ...
          html: "Reminder: if nothing is observed, every path is best.",
        }).appendTo(this.flowchartContainer);

        this.stageRow.addClass('broad');
        this.flowchart = this.createFlowchart(this.flowchart);
      }
    ...
    createFlowchart(flowchart){
      if(!jQuery.isEmptyObject(flowchart)){

        if(!flowchart.as_quiz) flowchart.as_quiz = false;
        if(!flowchart.quiz_random) flowchart.quiz_random = false;
        if(!flowchart.as_auto_select) flowchart.as_auto_select = false;
        if(!flowchart.auto_select_N) flowchart.auto_select_N = 3;
        if(!flowchart.quiz_random_N) flowchart.quiz_random_N = 3;

        if(!flowchart.force_interaction) flowchart.force_interaction = false;
        if(!flowchart.force_correct) flowchart.force_correct = false;
        if(!flowchart.show_feedback) flowchart.show_feedback = false;

        flowchart.index = 0;
        flowchart.level = 1;
        flowchart.parts = [];
        flowchart.trace = [];
        flowchart.left_button = null;
        flowchart.right_button = null;
        flowchart.click = function(){return null};
        flowchart.expand = function(){return null};
        flowchart.reset = function(){return null};
        flowchart.check = function(){return null};
        flowchart.recordQuery = function(){return null};
        flowchart.pred = {'onLevel': null, 'onBestPath': null, 'isUnobserved': null, 'onItsPathIs': null, 'onItsBestPathIs': null};
        flowchart.helper = {'getLevel': null, 'getPathNodes': null, 'getNodeValues': null, 'getBestPathTerms': null, 'getPathValueByTerm': null};
        flowchart.data = {
          index: {
            'target': [],
            'time': []
          ...
          answer_stats: {
            'total': Array(flowchart.images.length-3).fill(0),
            'correct': Array(flowchart.images.length-3).fill(0),
          }
        };


        // left button
        flowchart.left_button = $('<div>', {
          class: 'mouselab-flowchart-button',
          html: '<code>Yes</code>',
        }).on('mousedown', () => {
          return this.flowchart.click('left');
        }).appendTo(this.flowchartContainer).hide();

        // right button
        flowchart.right_button = $('<div>', {
          class: 'mouselab-flowchart-button',
          html: '<code>No</code>',
        }).on('mousedown', () => {
          return this.flowchart.click('right');
        }).appendTo(this.flowchartContainer).hide();


        // init image parts
        for(let index in flowchart.images){
          let flow_part = $('<img>', {
            src: flowchart.path + flowchart.images[index],
            class: 'mouselab-flowchart-part',
          }).appendTo(this.flowchartContainer);

          flow_part.css({left: 125, top: 0});
          if(index != 0){
            flow_part.hide();
          }
          flowchart.parts.push(flow_part);
        }

        //unfolding of flowchart
        flowchart.expand =   function(index, offset){
              if(index < this.parts.length){
                ...
                let offset_top = parent.height()*this.level;
                child.css({left: offset_left, top: offset_top});

                //buttons
                if(this.mappings[index].length > 0){
                  this.right_button.css({left: offset_left+ child.width()*(1-this.offsets[index]),  top: offset_top+ child.height()*0.52});
                  this.left_button.css({left:  offset_left+ child.width()* this.offsets[index] - this.left_button.width(), top: offset_top+ child.height()*0.52});
                ...
                if(index > 5 && this.as_auto_select){
                  ...
                  if(mdp.data.queries.preselect.state.target.length < this.auto_select_N){
                    setTimeout(function(){mdp.autoPreSelect();}, 100);
                  } else{
                    // end trial
                    this.reset(true);
                    mdp.complete = true;
        ...
        //button functions
        flowchart.click = function(choice) {
          ...
          if(!this.force_correct || check){
            ...
            if(this.quiz_random){
              if(this.data.index.target.length < this.quiz_random_N){
                let new_index = 0;
                do {
                  new_index = 1 + Math.floor(Math.random() * 5);
                } while (new_index == index);
                return this.expand(new_index, 1-this.offsets[new_index]);
              } else {
                return this.expand(6, 0.5);
              }
            }
        ...
        // record flowchart sequence
        flowchart.recordQuery = function(targetType, target) {
        ...
        flowchart.reset = function(logging = false) {
          if(this.index > 0){
        ...
        flowchart.check = function(choice){
          if(this.index > 0){
            ...
            for(let i in this.checks[this.index]){
              let ev = eval(this.checks[this.index][i]);
              ...
              if(ev.answer){
                ...
                feedback = answer? ev.text :  + ' and ' + ev.text;
                answer = false;
              }
            }
            // handle error: this node the path
            if(feedback.substring(0, 3) == 'the'){
        ...
        flowchart.pred.onLevel = function(s,neg,level){
          let answer = this.helper.getLevel(s) == level;
          let text = 'lies on level ' + String(this.helper.getLevel(s));

          return {'answer': this.helper.getNegate(answer, neg), 'text': text};
        };

        flowchart.pred.onBestPath = function(s,neg){
        ...
        flowchart.pred.isUnobserved = function(s,neg,value){
          let answer = !mdp.states[String(s)].revealed;
          let text = answer ? 'is unobserved' : "is observed";

          return {'answer': this.helper.getNegate(answer, neg), 'text': text};
        }

        flowchart.pred.onItsPathIs = function(s,neg,value){
          ...
          let text = answer ? "there's a path that passes through this node which contains a " + String(value) : "neither of the paths that pass through this node contain a " + String(value);

          return {'answer': this.helper.getNegate(answer, neg), 'text': text};
        }

        flowchart.pred.onItsBestPathIs = function(s,neg,value){
          let best_path_terms = this.helper.getBestPathTerms.call(null, s);
          let node_terms = mdp.findTermNodes(s);
          let answer = null;
        ...
        flowchart.helper.getNegate = function(answer,neg){
          return (neg ? !answer : answer);
        }

        flowchart.helper.getLevel = function(s){
          if([1,5,9].includes(s)){
            return 1;
        ...
        flowchart.helper.getPathNodes = function(s){
          let path_nodes = [];
        ...
        flowchart.helper.getNodeValues = function(S){
          let path_values = [];
        ...
        flowchart.helper.getBestPathTerms = function(s){
          let best_path_terms = [];
          let node_terms = mdp.findTermNodes(s);
        ...
        flowchart.helper.getPathValueByTerm = function(term){
    ...
    async move(s0, a, s1) {
      ...
      if (!this.moved) {
        ...
        if(this.show_interactive_flowchart) this.flowchart.reset(true);
    ...
    autoPreSelect(){
      let s = 1+ Math.floor(Math.random() * 12);
      this.preClickState(this.states[s], s);
    }
    ...
    async preClickState(g, s){
      ...
      if (this.stateLabels && this.stateDisplay === 'click' && !g.highlighted) {
        ...
        if(this.show_interactive_flowchart){
          ...
          this.flowchart.click('right');

          if(this.flowchart.force_interaction){
            fabric.Object.prototype.hoverCursor = 'default';
            this.freeze = true;
          }
        };
    ...
    async showFeedback(action) {
      var a, delay, highlight, i, info_feedback, len, loss, optimal, q, qs, ref, strictness, v, msg
      console.log('showFeedback');
      let with_info = true;
      let with_reward = true;
      ...
      this.prompt.html("<div id='feedback' style='color:#FF0000; font-weight:bold; font-size:15pt'/>");
      highlight = false;
      ...
      if (!mdp.showParticipant) {
        ...
        return this.keyListener = jsPsych.pluginAPI.getKeyboardResponse({
          ...
          callback_function: (info) => {
            ...
            let message = this.freeze ? 'Answer the questions first' : 'Invalid Direction'
    ...
    run() {
      ...
      fabric.Image.fromURL(this.playerImage, ((img) => {
        ...
        this.arrive(this.initial);
        if(this.show_interactive_flowchart && this.flowchart.as_auto_select){
          setTimeout(function(){mdp.autoPreSelect();}, 100);
        }
      }));
    ...
    buildMap() {
      var a, actions, height, location, maxx, maxy, minx, miny, r, ref, ref1, ref2, results, s, s0, s1, width, x, xs, xs2, y, ys, ys2, f;
      // Resize canvas.
      [xs, ys] = _.unzip(_.values(this.layout));

      //frederic modification notes
      if(this.show_path_rewards){
        [xs2, ys2] = _.unzip(_.values(this.notes_layout));
        xs = xs.concat(xs2)
        ys = ys.concat(ys2)
      }
      ...
      if(this.show_pre_select){
        ...
        rect.on('mousedown', () => {
          if(this.show_pre_select && !this.freeze) this.unhighlightAllNodes();
          if(this.show_interactive_flowchart  && !this.freeze) this.flowchart.reset();
        });
      ...
      this.notes = {};
      if(this.show_path_rewards){
        ref2 = removePrivate(this.notes_layout);
        for (s in ref2) {
          location = ref2[s];
          [x, y] = location;

          this.notes[s] = new Note(s, x - minx, y - miny, {
            fill: '#dcdcdc',
            label: '0'
          });
        }
    ...
    async endTrial() {
      ...
      if(this.showMoneyMade){
        clearInterval(this.feedback_timeout);
        this.lowerMessage.html("<b>You made <span class=mouselab-score/> on this round. </b> <br><span id=wait/>");
  ...
  State = class State {
    ...
    highlight() {
      ...
      return this.circle.set('fill', '#d26e6e');
    }
    unhighlight() {
      if(!this.revealed){
        this.setLabel('');
