...
jsPsych.plugins['mouselab-mdp'] = (function() {
  ...
  MouselabMDP = class MouselabMDP {
    constructor(config) {
      ...
      this.with_object_level_FB = false;
      ...
      if (this.energyLimit) {
        ...
        leftMessage = `Round ${this._block.trialCount + 1}/${this.round_length}`;
      }
      ...
      if ($('#mouselab-msg-right').length) { // right message already exists
        ...
        this.rightMessage2 = $('#mouselab-msg-right2');
        ...
        this.rightMessage2 = $('<div>', {
          id: 'mouselab-msg-right2',
          class: 'mouselab-header',
          html: ''
        }).appendTo(this.display);
        if (prompt !== null) {
    ...
    async clickState(g, s) {
      ...
      if (this.stateLabels && this.stateDisplay === 'click' && !g.label.text) {
        ...
        this.addScore(-this.stateClickCost, true);
    ...
    async showFeedback(action) {
      if(this.show_feedback){
        ...
        if (this._block.show_feedback) {
          ...
          if (loss === 0) {
            this.prompt.html("<div style='color:#008800; font-weight:bold; font-size:15pt'>\nGood job!\n</div>");
          } else {
            ...
            this.prompt.html(`<div  style='color:#FF0000; font-weight:bold; font-size:15pt'>\n${msg}\n</div>`);
            // @freeze = true
            // $('#mdp-feedback').show()
    ...
    arrive(s, repeat = false) {
      ...
      g = this.states[s];
      if(s != 0){
        this.revealRewards ? g.setLabel(this.stateRewards[s]) : g.setLabel('?');
      }
      this.canvas.renderAll();
      this.freeze = false;
      LOG_DEBUG('arrive', s);
    ...
    addScore(v, clicking=false) {
      ...
      if(this.revealRewards || clicking) this.drawScore(score);
      return;
    }
    ...
    drawScore(score) {
      if(this.blockName == 'testing'){
        this.rightMessage2.html('Bonus: ' + Math.max(0,round(calculateBonus()*100, 0)) + " cents");
      }
