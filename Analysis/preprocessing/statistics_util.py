...
def getDropoutStats(df1,df2, title):
    ...
    for c in df1.condition.unique().tolist():
        print('Percentage in condition {}: {}'.format(c,r(1-len(df2[df2.condition == c])/len(df1[df1.condition == c]))))
        print(' ' + str(len(df1[df1.condition == c])) + '/' + str(len(df2[df2.condition == c])))

def getKeyStats(df, key):
    print('')
    print(key)

    for c in df.condition.unique().tolist():
        tt = df[df.condition == c][key]
        print('Duration in condition {}: Mean: {}, Median: {}, Std: {}'.format(c, r(tt.mean(),1), r(tt.median(),1), r(tt.std(),1)))
...
def print_parametric_info(dfs, df_valid, key, demo = False):
    ...
    if dfs == None:
        dfs = []
        for c in df_valid.condition.unique().tolist():
                dfs.append(df_valid[df_valid.condition == c])
...
def print_LC(dfs, key, title='', ylabel = 'PA', xlabel = 'Trials', measure='median', labelkey = 'pid'):
    ...
    if measure == 'mean':
        ...
        for df in dfs:
            ...
            plt.errorbar(index + np.arange(len(df[key].iloc[0])), means, yerr=errors, fmt='-o', label = df[labelkey].iloc[0])
            index += 0.1
